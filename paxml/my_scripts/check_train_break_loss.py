...
def extract_files(path, check_whole_or_latest=None):
    read_file_nums = -2 if check_whole_or_latest == 'latest' else None
...
path = sys.argv[1]
check_whole_or_latest = sys.argv[2]
...
# path = 'gs://llm_projects/log/summaries/train/PilePythiaXLDynWFFN8HD64Win256Alignedv4'
# check_whole_or_latest = 'latest' # latest: 检查最新抢占点loss；whole: 检查全部抢占点loss
file_paths = extract_files(path, check_whole_or_latest=check_whole_or_latest)
...
# example:
# python check.py gs://llm_base_models/v5p_256/7B/summaries/train/PileDCSlimLlama7B4Kx4x256x1v5p latest