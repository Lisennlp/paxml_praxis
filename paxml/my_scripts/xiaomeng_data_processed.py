...
class DataProcessor():
    def __init__(self, data_pathfile, tokenizer, save_dir, data_type='zh', max_seq_len=2048, ratio=0.5, shuffle=True):
        bucket = True
        if bucket:
            self.path_map = {
    ...
    def extract_filepath(self):
        self.books_pathlist = [line.strip().replace(*self.path_map[self.data_type])
...
#         save_path = os.path.join(self.save_dir, f'{self.data_type}_BOOK_{rank}')
    ...
    def run(self, start, end, rank):
        ...
        print(
            f'Rank: {rank}. book: {self.books_pathlist[:10]}. combine_nums: {self.combine_nums}')
        for index, path in enumerate(self.books_pathlist[start: end]):
            if index % self.combine_nums == 0:
                print(
                    f'rank: {rank} index: {index} self.combine_nums: {self.combine_nums}')
                save_path = os.path.join(
                    self.save_dir, f'{self.data_type}_BOOK_{rank}_{index}')
                print(
                    f'\n\n\n\n\n================Rank: {rank} save_path: {save_path} =====================')
                try:
                    writer.close()
            ...
            time_end = time.time()
            print(
                f'{rank}-processed: {index}/{N}, path: ‘{path}’ deal finished, take: {time_end - time_start}.')
            if rank == 0:
                wandb_stats = {'index': index, 'N': N,
...
def process_book_wrapper(args):
    ...
    bucket = True
    if bucket:
        data_pathfiles = {'zh': 'gs://jax_llm_data/xiaomeng/zh_data/69shuba.filelist.shuffled',
                          'en': 'gs://jax_llm_data/xiaomeng/en_data/allfile.filelist.shuffed'}
    else:
        data_pathfiles = {'zh': '/nas2/xiaomeng/zh_data/69shuba.filelist.shuffled',
    ...
    processor = DataProcessor(data_pathfiles[LANG],
                              tokenizer_path,
                              save_dir,
                              data_type=LANG,
    ...
    if rank == 0:
        ...
        wandb.init(project='xiaomeng', name='data_processed',
                   config=None, resume=True)
...
#     os.makedirs(save_dir, exist_ok=True)
    ...
    args = ([rank, LANG, WORKERS, max_seq_len, ratio]
            for rank in range(WORKERS) for LANG in ['en', 'zh'])
