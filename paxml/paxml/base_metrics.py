...
def _pmap_aggregate_metrics(f, batch_metrics, metric_keys, reshard: bool):
    ...
    def _reshard(batch_metrics):
        ...
        for k, v in batch_metrics.items():
            ...
            new_weight = (
                jnp.ones(shape=(jax.local_device_count(),), dtype=weight.dtype)
                * weight
    ...
    if reshard:
        pmap_aggregate = jax.pmap(_aggregate, axis_name=PMAP_PARALLEL_AXIS_NAME, out_axes=None)
...
class BaseMetrics(base_hyperparams.FiddleBaseParameterizable, metaclass=abc.ABCMeta):
    ...
    def summarize(self, step_i, prefix):
        ...
        for k, v in metrics.items():
            ...
            summary_utils.write_summary_tensor(
                step_i, f"{prefix}/{k}", value, summary_utils.SummaryType.AGGREGATE_SCALAR
            )
            summary_utils.write_summary_tensor(
                step_i, f"{prefix}/{k}-weight", weight, summary_utils.SummaryType.AGGREGATE_SCALAR
            )
        return metrics
...
class MeanMetrics(BaseMetrics):
    ...
    def aggregate(self, batch_metrics, reshard: Optional[bool] = False):
        ...
        return _pmap_aggregate_metrics(_pmap_mean, batch_metrics, self.metric_keys, reshard)
...
class MaxMetrics(BaseMetrics):
    ...
    def aggregate(self, batch_metrics, reshard: Optional[bool] = False):
        ...
        return _pmap_aggregate_metrics(_pmap_max, batch_metrics, self.metric_keys, reshard)
...
class HistogramMetrics(BaseMetrics):
    ...
    def aggregate(self, batch_metrics, reshard: Optional[bool] = False):
        ...
        return _pmap_aggregate_metrics(_pmap_sum, batch_metrics, [self.histogram_key], reshard)
    ...
    def finalize(self):
        ...
        for k in self._metrics.keys():
            ...
            metrics[k + "_coverage"] = (avg_num_covered_words / num_classes, sum_metric_weights)
        return metrics
...
class LossAggregator(base_hyperparams.FiddleBaseParameterizable):
    ...
    def aggregate(self, batch_metrics) -> Tuple[float, float, Union[float, None]]:
        ...
        # batch_metrics keys: 'total_loss', 'avg_xent', 'aux_loss', 'log_pplx', 'fraction_of_correct_next_step_preds', 'num_predictions'
