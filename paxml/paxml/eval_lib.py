...
class _PmapEvalCheckpointer(_EvalCheckpointer):
    ...
    def _restore(self, step: int, train_state_global_shapes: TrainState) -> TrainState:
    ...
    def load_checkpoint_for_step(
        ...
        model_states = self._restore(step, train_state_metadata.unpadded_global_shapes)
...
def evaluate(
    ...
    reshard_inputs = (
        ...
        or eval_input_p[0].experimental_remote_input
    )
    partitioner = partitioning.create_partitioner(
        ...
        auto_sharding_input_params=eval_input_p[0] if enable_auto_sharding else None,
    )
...
class _EvalRunner:
    ...
    def setup_decode_programs(self, output_pickle):
        ...
        trainer_lib.check_unique_names([p.decode_input for p in self._decode_programs])
...
def decode(
    ...
    reshard_inputs = (
        checkpoint_type != CheckpointType.PERSISTENCE
        or combined_input_ps[0].experimental_remote_input
    )
    partitioner = partitioning.create_partitioner(
        jax_task,
        init_is_eval=True,
    ...
    wait_until_step = (
        jax_task.hparams.train.decode_start_after_n_steps if continuous_decode else None
    )
    checkpointer = _create_checkpointer(
        jax_task=jax_task,
        job_log_dir=job_log_dir,
...
def _eval_or_decode(
    *,
    ...
    job_log_dir: epath.Path,
    eval_runner: _EvalRunner,
) -> Tuple[Optional[tuning_lib.EvalMetrics], Optional[tuning_lib.DecodeMetrics]]:
...
def _common_eval_or_decode_loop(
    *,
    ...
    logging.info("Evaluation loop starting from step `%d`...", last_checkpoint_step)
...
def infer_and_write(
    ...
    partitioner = partitioning.create_partitioner(task, reshard_inputs=reshard_inputs)
    ...
    partitioner.setup(task, prng_key, train_input_specs, input_for_shape, job_log_dir)
...
def infer_and_write_pmap(
    ...
    for input_gen in inputs:
        ...
        num_steps = -1 if input_gen.reset_for_eval else input_gen.eval_loop_num_batches
