    ...
    *,
    ...
    while True:
        ...
        if not train_program.should_run(partitioned_train_state, step_i):
            ...
            break
        # lsp:trainer_lib.py:(return new_states, StepFnOutput()) ->  programs.py: (new_step, new_state, train_outputs = self.train_step()) -> here
        ...
        if train_p.eval_interval_steps and step_i % train_p.eval_interval_steps == 0:
        ...
        logging.log_first_n(INFO, "[PAX STATUS]: Step `%d` completed.", 5, step_i - 1)
        # loss 低于某个值自动停止
