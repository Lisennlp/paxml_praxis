...
class Learner(base_hyperparams.FiddleBaseParameterizable):
    ...
    bprop_variable_exclusion: Union[str, Sequence[str]] = pax_fiddle.instance_field(
        default_factory=list
    )
    bprop_variable_inclusion: Union[str, Sequence[str]] = pax_fiddle.instance_field(
        default_factory=list
    )
    ...
    def __post_init__(self):
        assert self.name, 'Learner params for %s must have a "name"' % self.__class__.__name__
        ...
        if not self.vectorize_on_repeat_prefix:
            assert (
                not self.force_repeat_prefix_structure
            ), "force_repeat_prefix_structure requires vectorize_on_repeat_prefix=True"

    def plot_learning_rate(self, step: int) -> None:
        learning_rate = self.optimizer.get_learning_rate(
            step
        )  # pytype: disable=wrong-arg-types  # jax-ndarray
        base_layer.add_global_summary("lr", learning_rate, SummaryType.AGGREGATE_SCALAR)
        base_layer.add_global_summary("learning/lr", learning_rate, SummaryType.AGGREGATE_SCALAR)
    ...
    def scale_gradients(
        ...
        if clip_gradient_single_norm_to_value is None:  # 0
            clip_gradient_single_norm_to_value = self.optimizer.clip_gradient_single_norm_to_value
    ...
    def apply_gradient(
        ...
        return jax.tree_util.tree_map(_adjust_var, old_vars, transformed_grads, var_is_learnable)
...
class MultiOptimizerLearner(Learner):
    ...
    auxiliary_optimizers: Sequence[pax_fiddle.Config[optimizers.BaseOptimizer]] = ()
    ...
    def __post_init__(self):
        ...
        self._auxiliary_optimizer_insts = [instantiate(opt) for opt in self.auxiliary_optimizers]
    ...
    def plot_learning_rate(self, step: int) -> None:
        ...
        for name, optimizer in zip(self.auxiliary_names, self._auxiliary_optimizer_insts):
    ...
    def get_masks(
        ...
        for regex, grad_tx_fn in zip(self.auxiliary_regex, self._auxiliary_grad_tx_fn):
            regexp = re.compile(regex)
            prefix = py_utils.extract_prefixed_keys_from_nested_map(var_weight_hparams)
            mask = jax.tree_map(lambda x, regexp=regexp: regexp.match(x) is not None, prefix)
        ...
        def check_var_in_auxiliary_regex(*args):
            ...
            for x in args:
                if x:
                    ...
                    if r:
                        raise ValueError(
                            "The regex pattern of auxiliary optimizers shouldbe non-overlapping."
                        )
                    r = True
            return r
    ...
    def get_grad_tx(
        ...
        for mask, grad_tx_fn in zip(optimizer_mask, self._auxiliary_grad_tx_fn):
            optimizer_chain.append(
                optimizers.sharded_masked(grad_tx_fn(var_weight_hparams, include_ema=False), mask)
            )
        ...
        op = self.optimizer
        if op.ema_decay > 0.0:
            optimizer_chain.insert(0, optimizers.apply_ema_weights(decay=op.ema_decay))
    ...
    def update_states(
        ...
        if self.apply_separate_scaling:
            grads, valid_step = self.scale_gradients_by_optimizer(grads, var_weight_hparams)
        else:
            grads, valid_step = self.scale_gradients(grads)
        ...
        if self.enable_skip_step_on_gradient_anomalies:
            ...
            # Keep the old state if the step is invalid.
            def _update(x, y):
                if not py_utils.is_optax_masked_node(x) and not py_utils.is_optax_masked_node(y):
