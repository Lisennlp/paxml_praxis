...
flags.DEFINE_string("server_addr", None, help="server ip addr")
...
def run(
    ...
    if FLAGS.jax_profiler_port is not None:
        server = jax.profiler.start_server(
...
def _main(argv: Sequence[str]) -> None:
    ...
    if FLAGS.exp is not None:
        ...
        cfg = absl_flags.create_buildable_from_flags(module=None, allow_imports=True)
...
if __name__ == "__main__":
    ...
    if handle := os.getenv("BORG_TASK_HANDLE"):
        if (
            task_id := _TASK_HANDLE_RE.match(handle).group(1)
        ) == "0":  # pytype: disable=attribute-error  # re-none
