...
def filter_nestedmap(full_specs, partial_specs):
    ...
    if isinstance(full_specs, dict):
        ...
        for (
            key
        ) in partial_specs.keys():  # pytype: disable=attribute-error  # jax-ndarray
        ...
        result = [
            filter_nestedmap(lhs, rhs) for lhs, rhs in zip(full_specs, partial_specs)
        ]
        return result
    else:
        return full_specs
...
def _write_input_specs(
    ...
    work_unit.create_artifact(platform.ArtifactType.FILE, str(fpath), "Input specs")
...
class PjitPartitioner(Partitioner):
    ...
    def __init__(
        ...
        init_is_eval: bool,
        reshard_inputs: bool,
        task: Union[pax_fiddle.Config[tasks_lib.SingleTask], tasks_lib.SingleTask],
        ...
        if not self._broadcast_key_fn:
            ...
            def _broadcast_key(k):
                ...
                with self._global_mesh:
                    return pjit.pjit(_identity, in_shardings=None, out_shardings=None)(
                        k
                    )
    ...
    def _pad_states(self, metadata: TrainStateMetadata, unpadded_state: TrainState):
    ...
    def _unpad_states(self, metadata: TrainStateMetadata, padded_state: TrainState):
...
class AutoShardingPjitPartitioner(PjitPartitioner):
    ...
    def _get_train_inputs_shape_dtype(
        ...
        global_shape_dtype = super()._get_train_inputs_shape_dtype(train_input_pipeline)
    ...
    def get_train_state_metadata(
        ...
        logging.info("Running auto sharding for %r", self._auto_sharding_info.step_fn)
        wrapped_step_fn = self._get_step_fn(
