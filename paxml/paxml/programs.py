...
def get_eval_train_state(task: tasks_lib.SingleTask, state: TrainState, use_ema: bool):
...
class BaseTrainProgram(Program):
    ...
    def run(self, state: TrainState, step: int) -> TrainProgramOutput:
        ...
        logging.log_first_n(logging.INFO, "[PAX STATUS]:  Performing train_step().", 5)
...
class SingleTaskTrainProgram(BaseTrainProgram):
    ...
    def _get_train_step(
        ...
        if not self._train_step_created:
            (
                self._train_step_fn,
                self._train_step_input_partition_spec,
...
class BaseEvalProgram(Program):
    ...
    def setup(
        ...
        self._eval_unpadded_global_batch_size = (
            self._eval_input_pipeline.get_global_batch_size(self._eval_input_pipeline)
        )
        # lsp: -1时表示评测整个eval集， reset_for_eval为True的时候为True
        self._eval_num_steps = (
            -1 if self._input_p.reset_for_eval else self._input_p.eval_loop_num_batches
        )
        # lsp
    ...
    def run(self, state: TrainState, step: int) -> EvalProgramOutput:
        ...
        output_dir = self._job_log_dir / f"{EvaluationMode.EVAL.value}_out" / self._name
