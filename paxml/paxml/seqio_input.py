...
def _update_keys(
    ...
    for k in targets:
        if k not in answers:
            ...
            if len(new_keys) != 1:
                raise ValueError(
                    f'key="{k}" in targets matches to {len(new_keys)} '
...
def _log_plain_text_output(
    answers: Mapping[str, NestedMap], plain_text_output: TextIO
) -> None:
...
def _enumerate_dataset(
    ds: tf.data.Dataset, is_training: bool, shard_info: Optional[seqio.ShardInfo]
) -> tf.data.Dataset:
...
def maybe_update_decode_output_keys(
    ...
    """
    if not (
        enum_key_fields := py_utils.filter_by_matching_keys(
            decode_out, [py_utils.PROVENANCE_PREFIX]
        )[0]
    ):
...
def process_outputs(
    ...
    plain_text_output_fname: Optional[str] = None,
) -> Dict[str, float]:
    ...
    merged_seqio_metrics = {
        ...
        if not isinstance(v, seqio.metrics.MetricValue)
        else v
        for k, v in merged_seqio_metrics.items()
    }
    logger(
        task_name=inp.mixture_or_task_inst.name,
        step=step,
        metrics=merged_seqio_metrics,
        dataset=None,
        inferences=None,
        targets=None,
    )
...
class SeqIOInput(base_input.BaseInput):
    ...
    _cached_targets_with_enum_key: Optional[
        Mapping[str, NestedMap]
    ...
    def __post_init__(self):
        ...
        if not self.name:
            mixture_name = (
                self.mixture_name or self.mixture_or_task.name
    ...
    def _validate_hparams(self):
        ...
        self._mixture_or_task_inst = self.mixture_or_task or seqio.get_mixture_or_task(
            self.mixture_name
        )
        self._shard_info = self._validate_deterministic()

        if not self.is_training and isinstance(self.mixture_or_task_inst, seqio.Task):
    ...
    def _get_num_eval_examples(self) -> int:
        ...
        if (
            ...
            return self.eval_loop_num_batches * self.batch_size * self.num_infeed_hosts
        return -1
    ...
    def _gen_targets_iter(self) -> None:
        ...
        if not self.use_enumeration:
            self.targets_iter_converted = self.targets_ds_converted.as_numpy_iterator()
    ...
    def save(self, checkpoint_path: epath.PathLike):
        try:
            ...
            raise NotImplementedError(
                "Checkpointing is not supported for this SeqIO input"
            ) from e
    ...
    # lsp
    def _get_backing_ds(
        self, shuffle: bool, num_epochs: int, shard_info: Optional[seqio.ShardInfo]
    ) -> tf.data.Dataset:
        kwargs = dict(
    ...
    def _pad_to_batch_size(self, ds: tf.data.Dataset) -> tf.data.Dataset:
        ...
        logging.info(
            ...
            "%d extra entries to get %d batches.",
            local_num,
            pad_num,
            num_batches,
        )
    ...
    def _build_predict_metric_inputs_with_prefix(
        ...
        num_examples = 0
        while self._num_eval_examples < 0 or num_examples < self._num_eval_examples:
            num_examples += 1
            try:
            ...
            key = self.ids_to_strings(inputs, lengths=[inputs_length], key="src")[
                0
            ]  # pytype: disable=wrong-arg-types
            t = _get_targets_str(example, self.mixture_or_task_inst)
            targets[key].append(
                self.task_inst.postprocess_fn(t, example=example, is_target=True)
            )
    ...
    def _build_predict_metric_inputs_with_enum(
        ...
        for k in targets:
            ...
            ans["seqio_postprocessed_predictions"] = _convert_bytes_to_str(prediction)
            ...
            for e in targets[k]:
                ...
                if verbose_entries_idx < verbose_entries:
                    logging.info(
                        ...
                        e.get("targets_pretokenized", "None"),
                        e.get("is_correct", "N/A"),
                        target_post,
                        score,
                    )
    ...
    def compute_metrics(
        ...
        if is_packing_on(self.feature_converter):
            logging.error(
                "Will not compute metrics on %s since using a "
                "FeatureConverter with pack=True.",
                task.name,
            )
            return []
...
class LanguageModelFeatures(
    seqio.DecoderFeatureConverter, base_hyperparams.StrOverride
):
    ...
    def _to_pax(self, b) -> NestedMap:
        ...
        if hasattr(b, "decoder_causal_attention"):
            non_padding = tf.math.logical_or(
                b.decoder_causal_attention > 0, non_padding
            )
...
class SequenceModelFeatures(seqio.EncDecFeatureConverter):
    ...
    def __init__(
        self, pack: bool = False, use_custom_packing_ops: bool = False, bos_id: int = 0
    ) -> None:
        self._pack = pack
        self._use_custom_packing_ops = use_custom_packing_ops
        self._bos_id = bos_id
        super().__init__(
            pack=pack, use_custom_packing_ops=use_custom_packing_ops, bos_id=bos_id
        )
...
def get_eval_hparams_for_seqio(
    ...
    if isinstance(task_or_mixture_name, (seqio.Task, seqio.Mixture)):
        ...
        raise TypeError(
            "Expected `task_or_mixture_name` to be a string, a Task, "
