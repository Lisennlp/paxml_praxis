...
flags.DEFINE_bool(
    "pax_only_aggregate_summaries",
    False,
    "If true, only output aggregate summary types (AGGREGATE_SCALAR, AGGREGATE_IMAGE).",
)
FLAGS = flags.FLAGS
...
def pretty_repr_shapes(replicated_vars: NestedJTensor, is_vars_replicated) -> str:
...
def pretty_repr_provenance(provenance: Union[TensorProvenance, Nested[TensorProvenance]]) -> str:
...
def _yield_subtrees(
    ...
    if level < max_level:
        if isinstance(root, dict):
            for key in root:
                for out in _yield_subtrees(root[key], max_level, level + 1, name + (key,)):
                    yield out
        elif isinstance(root, (list, tuple)):
            list_len = len(root)
            for ii in range(list_len):
                for out in _yield_subtrees(root[ii], max_level, level + 1, name + (str(ii),)):
                    yield out
        else:
...
def flatten_flax_summaries(summary_tensors: NestedJTensor) -> Dict[str, JTensor]:
    ...
    for k, v in summary_tensors.items():
        ...
        if isinstance(v, list):
            for i, u in enumerate(v):
                ...
                full_name = (
                    root_name + "_" + str(i) + base_layer.get_summary_type_suffix(summary_type)
                )
...
def aggregate_per_replica_summaries(summary_tensors: NestedJTensor):
    ...
    scalar_summaries = jax.lax.pmean(scalar_summaries, axis_name=PMAP_PARALLEL_AXIS_NAME)
    # Gather per-replica image results.
    image_summaries = jax.tree_map(
        lambda x: jax.lax.all_gather(x, axis_name=PMAP_PARALLEL_AXIS_NAME), image_summaries
    )
    max_entries = MAX_IMAGES_PER_SUMMARY
    image_summaries = jax.tree_map(
        lambda x: jnp.reshape(x, [-1] + list(x.shape)[-3:])[:max_entries], image_summaries
    )
    audio_summaries = jax.tree_map(
        lambda x: jax.lax.all_gather(x, axis_name=PMAP_PARALLEL_AXIS_NAME), audio_summaries
    )
    max_entries = MAX_AUDIOS_PER_SUMMARY
    audio_summaries = jax.tree_map(
        lambda x: jnp.reshape(x, [-1] + list(x.shape[-2:]))[:max_entries], audio_summaries
    )
    video_summaries = jax.tree_map(
        lambda x: jax.lax.all_gather(x, axis_name=PMAP_PARALLEL_AXIS_NAME), video_summaries
    )
...
def flatten_summary_dict(
    summary_dict: Dict[str, JTensor], parent_key: Optional[str] = None
) -> List[Any]:
...
def write_summary_entry(
    ...
    weighted_scalars_list = py_utils.maybe_unreplicate_for_fully_replicated(weighted_scalars_list)
    ...
    with summary_writer.as_default():
        ...
        write_summary_tensor(step_i, "loss", mean_loss, SummaryType.AGGREGATE_SCALAR)
        if steps_per_sec is not None:
            status_msg += f", steps/sec {steps_per_sec}"
            write_summary_tensor(step_i, "Steps/sec", steps_per_sec, SummaryType.AGGREGATE_SCALAR)
        ...
        for key, value_lst in weighted_scalars_list.items():
            sum_metric_weights = 0.0
            weighted_sum_metric_values = 0.0
            for value in value_lst:
                assert len(value) == 2, "Metric value should be a pair of (value, weight)."
            ...
            logging.info("  %s=%f (weight=%f)", key, weighted_average, sum_metric_weights)
            status_msg += f", {key}={weighted_average}"
            write_summary_tensor(
                step_i, f"Metrics/{key}", weighted_average, SummaryType.AGGREGATE_SCALAR
            )
            write_summary_tensor(
                step_i, f"Metrics/{key}-weight", sum_metric_weights, SummaryType.AGGREGATE_SCALAR
            )
    ...
    logging.info("Wrote summary entry at step `%d` (loss=`%f`).", step_i, mean_loss)
...
def write_total_num_params(train_summary_writer: SummaryWriter, total_num_params: int):
...
def write_global_batch_size(train_summary_writer: SummaryWriter, global_batch_size: int):
...
class SummaryHandler:
    ...
    def should_accumulate(self, step: int) -> bool:
        ...
        return self.accumulate_over_steps and step % self._accumulate_interval_steps == 0
    ...
    def process(
        ...
        if not (should_log or should_accumulate or should_write_summary):
            ...
            return False
        loss = py_utils.maybe_unreplicate_for_fully_replicated(loss)
        weighted_scalars = py_utils.maybe_unreplicate_for_fully_replicated(weighted_scalars)
        summary_tensors = py_utils.maybe_unreplicate_for_fully_replicated(summary_tensors)
        ...
        if self._summary_pool:
            ...
            def process_fn():
                ...
                (loss_copy, weighted_scalars_copy, summary_tensors_copy) = jax.device_get(
    ...
    def _process(
        ...
        if False and should_log:  # XD
            logging.info("[PAX STATUS] step_i: %d, %s loss: %s", step, self._name, loss)
