...
def adjust_input_params_for_small_batch(
    ...
    if copy.infeed_host_index >= input_p.num_infeed_hosts:
        logging.info("Process %s: infeed data will be dropped.", jax.process_index())
    ...
    for global_device_idx, device in enumerate(global_mesh.devices.flat):
        ...
        if process_idx >= input_p.num_infeed_hosts or core_offset_in_host >= batch_size:
...
def _maybe_to_bfloat16_vars(mdl_vars, var_weight_hparams):
    ...
    return jax.tree_util.tree_map(_maybe_bfloat16_var_fn, mdl_vars, var_weight_hparams)
...
def _maybe_synchronize_non_learnable_vars(old_vars, new_vars, var_weight_hparams):
    ...
    def _synchronize_vars_using_mean(old_var: JTensor, new_var: JTensor) -> JTensor:
    ...
    def _synchronize_vars_using_sum(old_var: JTensor, new_var: JTensor) -> JTensor:
    ...
    if base_layer.is_running_under_pmap():
        ...
        return jax.tree_util.tree_map(_sync_var, old_vars, new_vars, var_weight_hparams)
...
def train_step_single_learner(
    ...
    *,
    ...
    prng_key = jax.random.fold_in(
        prng_key, states.step
    ...
    # lsp: None
    if grad_fn is None:
        ((weighted_loss, aux_info), grads), inter_values = GradFn().apply(
            {}, updated_mdl_vars, inputs, subkey, mutable=["intermediates"]
        )
    else:
    ...
    with base_layer.JaxContext.new_context(hparams=context_p):
        ...
        # lsp: 更新trainstate
        new_states = states.new_state(mdl_vars=mdl_vars, opt_states=[new_opt_states])
...
def _decode_step_for_partitioner(
    ...
    (weighted_scalars, per_example_out, updated_metrics), updated_vars = decode_step(
    ...
    if base_layer.is_running_under_pmap():
        ...
        summary_tensors = summary_utils.aggregate_per_replica_summaries(summary_tensors)
...
def initialize_partitioned_model_states(
    ...
    train_state_provenance = train_states.build_train_state_provenance(partitioned_vars)
    # Overwrite some parts if init_checkpoint_rules are set (warm-start)
    if do_init_checkpoint_rules and jax_task.hparams.train.init_from_checkpoint_rules:
