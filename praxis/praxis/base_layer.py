...
def var_partition_specs(
    ...
    def _get_spec(var_p: WeightHParams) -> jax.sharding.PartitionSpec:
        # lsp: 每个参数都是WeightHParams类的对象，有一个函数full_split_dims_mapping，直接获取该参数的shard
...
def maybe_shard(
    ...
    unconstrained_dims: Optional[Sequence[int]] = None,
) -> JTensor:
...
class WeightHParams:
    ...
    def full_split_dims_mapping(self) -> SplitDimsMapping:
        split_dim_mapping = self.tensor_split_dims_mapping or (-1,) * len(self.shape)
...
def scaled_orthogonal(key: JTensor, shape: Sequence[int], dtype: jnp.dtype = jnp.float32):
    """Scaled orthogonal initialization."""
    scale = max(np.sqrt(float(shape[-2]) / shape[-1]), 1)
    ortho_init = jax.nn.initializers.orthogonal(scale=scale, column_axis=-1, dtype=dtype)
    return ortho_init(key, shape)


def scaled_delta_orthogonal(key: JTensor, shape: Sequence[int], dtype: jnp.dtype = jnp.float32):
...
def init_var(var_p: WeightHParams, prng_key: PRNGKey, var_full_name: str) -> JTensor:
    """Creates an initial value of a var."""
    assert var_p is not None and var_p.init is not None
    # lsp：之前传递的params_init -> method 其实就是name || scale -> scale ： gaussian: 0.0006. xavier: 1.00001
    method = var_p.init.method
    scale = var_p.init.scale
    assert isinstance(scale, (int, float))
    shape = var_p.shape
    init_dtype = var_p.dtype
    fan_in_axes = var_p.fan_in_axes  # 参数的输入dim
    fan_out_axes = var_p.fan_out_axes  # 参数的输出dim
    ...
    if shape:
        assert all([dim_size > 0 for dim_size in shape]), "shape of %s is %s" % (
            var_full_name,
            shape,
        )
        dim0 = shape[0]
    ...
    if method in ["gaussian_sqrt_dim", "uniform_sqrt_dim", "truncated_gaussian_sqrt_dim"]:
        if len(shape) > 2:
    ...
    if method in [
        ...
        "gaussian_sqrt_fanavg",
    ]:
        # lsp: 高斯初始化 random.normal, scale
        return scale * jrandom.normal(prng_key, shape, init_dtype)
    elif method in ["uniform", "uniform_sqrt_dim"]:
        return scale * jrandom.uniform(prng_key, shape, init_dtype, minval=-1.0, maxval=1.0)
        ...
        return limit * jrandom.uniform(prng_key, shape, init_dtype, minval=-1.0, maxval=1.0)
        ...
        return max_val * jrandom.uniform(prng_key, shape, init_dtype, minval=-1.0, maxval=1.0)
    else:
...
class BoxedParam(struct.PyTreeNode, AxisMetadata):
    ...
    def __post_init__(self):
        assert not isinstance(self.value, BoxedParam), "Cannot double-box a parameter!"
    ...
    def add_axis(self, index: int, metadata_params: Dict[Any, Any]) -> TAxisMetadata:
        if index != 0:
        ...
        if self.meta.fan_in_axes:
            new_meta.fan_in_axes = [i if i < 0 else i + 1 for i in self.meta.fan_in_axes]
        if self.meta.fan_out_axes:
            new_meta.fan_out_axes = [i if i < 0 else i + 1 for i in self.meta.fan_out_axes]
        return self.replace(meta=new_meta)

    def remove_axis(self, index: int, metadata_params: Dict[Any, Any]) -> TAxisMetadata:
        if index != 0:
            raise ValueError("Only index==0 is implemented; given index=", index)
        if not metadata_params["is_initializing"]:
            return self
...
def unbox_meta(tree):
    ...
    return jax.tree_map(lambda bp: bp.meta, tree, is_leaf=lambda x: isinstance(x, BoxedParam))
...
class _SummaryDict:
    ...
    def add_summary(self, name: str, tensor: JTensor, summary_type: SummaryType) -> None:
...
# garbage-collected.
_SharedLayerCacheEntry = collections.namedtuple(
    "_SharedLayerCacheEntry", ["layer", "hparams", "wrapper"]
)
...
class JaxContext:
    ...
    @staticmethod
    def new_context(*, hparams: Optional[JaxContext.HParams] = None) -> JaxContext:
    ...
    ) -> Optional[_SharedLayerCacheEntry]:
        logging.info(
            "lookup_shared_layer called with id: %s in the scope of %s", shared_layer_id, root_scope
        )
        return self._root_scope_to_shared_layers_map[root_scope][shared_layer_id]

    def set_shared_layer(
        self,
        root_scope: flax_core.Scope,
        shared_layer_id: str,
        wrapper: _WrapperLayer,
        layer_hparams,
    ):
        logging.info(
            "set_shared_layer called with id: %s in the scope of %s", shared_layer_id, root_scope
        )
        existing = self.lookup_shared_layer(root_scope, shared_layer_id)
        assert existing is None
        self._root_scope_to_shared_layers_map[root_scope][shared_layer_id] = _SharedLayerCacheEntry(
...
def add_global_summary(
    name: str, tensor: JTensor, summary_type: SummaryType = SummaryType.SCALAR, verbosity: int = 2
) -> None:
...
class Theta:
    ...
    def __getattr__(self, k):
        ...
        if self.module.fprop_dtype == jnp.bfloat16 and var_disallow_bfloat16_conversion(
            var_hparams
        ):
            return variable
...
def instantiate_layer(layer_p: pax_fiddle.Config, scope: Any) -> BaseLayer:
    ...
    if layer_p.shared_weight_layer_id:
        ...
        pre_created = jax_context.lookup_shared_layer(scope, layer_p.shared_weight_layer_id)
        if pre_created is not None:
            assert compatible_hparams(pre_created.hparams, layer_p), (
                "shared layers are of incompatible configs "
            ...
            layer = wrapper.cld
            jax_context.set_shared_layer(
                scope, layer_p.shared_weight_layer_id, wrapper, layer_p.clone()
            )
    else:
        # simply create the child # lsp: 实例化参数层
...
class _FiddleHParamsClassStub(type, base_hyperparams.OverrideSubConfigFieldProtocol):
...
def is_sublayer_template_or_instance(val):
    ...
    if isinstance(val, (list, tuple)):
        if any(is_sublayer_template_or_instance(child) for child in val):
            if all(is_sublayer_template_or_instance(child) or child is None for child in val):
                return True
    return False


# lsp: create_variable 调用 self.params_init 函数 去初始化
class BaseLayer(nn.Module):
    ...
    * Define a __call__() method, which carries out the ML computation.
    ...
    def get_shard_count_on_dim(self, dim_sharding: Optional[Union[str, Sequence[str]]]) -> int:
    ...
    def _override_split_dim_mapping_fields(cls):
        ...
        if "WeightSharding" in cls.__dict__:
            if not issubclass(cls.WeightSharding, BaseLayer.WeightSharding):
                raise ValueError(
                    f"Expected {cls}.WeightSharding to be a subclass of BaseLayer.WeightSharding"
                )
            if not typing.TYPE_CHECKING:
                dataclasses.dataclass(frozen=True)(cls.WeightSharding)
            # TODO(b/249483164): Change this to use instance_field rather than
            # template_field after the Fiddle migration.
            cls.__annotations__["weight_split_dims_mapping"] = pax_fiddle.Config[cls.WeightSharding]
    ...
    def init(self, rngs, *args, method=None, mutable=None, **kwargs):
        ...
        variables = super().init(rngs, *args, method=method, mutable=mutable_list, **kwargs)
    ...
    def abstract_init_with_metadata(
        ...
        variables_abstract = self._abstract_init(*args, do_eval=do_eval, method=method, **kwargs)
        # args: train_shape_dtype
    ...
    def add_summary(
        ...
        if JaxContext.has_context() and verbosity > self.jax_context.summary_verbosity:
            return
    ...
    # lsp：初始化参数函数
    @nn.nowrap
    def create_variable(self, name: str, var_hparams: WeightHParams, trainable: bool = True) -> Any:
        ...
        if var_hparams.init is None:
            # lsp: 每个参数的init函数其实就是self.params_init
        ...
        if self.mesh_shape is not None:
            if (
                len([dim for dim in var_hparams.shape if dim > 1]) > 1
                and var_hparams.tensor_split_dims_mapping is None
            ):
                logging.warning(
            ...
            if var_hparams.tensor_split_dims_mapping is not None:
                assert len(var_hparams.tensor_split_dims_mapping) == len(var_hparams.shape)

        if var_hparams.collections is None:
            var_hparams.collections = []

        if (
            self.skip_lp_regularization
            and WeightHParamsCollection.SKIP_LP_REGULARIZATION not in var_hparams.collections
        ):
            var_hparams.collections = var_hparams.collections + [
                WeightHParamsCollection.SKIP_LP_REGULARIZATION
            ]

        if (not trainable) and (
            WeightHParamsCollection.NON_TRAINABLE not in var_hparams.collections
        ):
            var_hparams.collections = var_hparams.collections + [
    ...
    def create_children(
        ...
        *,
        ...
        self._check_child_layername_conflict(name)
        children = [
            self._create_child(f"{name}_{next(uid)}", p, skip_clone=skip_clone) for p in params
        ]
        if self._state.in_setup:
    ...
    def _create_child(
        ...
        if not isinstance(params, pax_fiddle.Config):
            msg = f"Expected templates for `create_child` to be Fiddle Configs; got {type(params)}."
        ...
        if name is self._private_children:
            raise ValueError(f"Child `{name}` already exists: make sure to use unique child names.")
    ...
    def _check_child_layername_conflict(self, name: str):
        ...
        if name in self._hparam_fields:
            raise AttributeError(
                ...
                "it already has a field with that name."
            )
...
def _is_template_type(typ):
    ...
    if origin is None and isinstance(typ, type) and issubclass(typ, pax_fiddle.Config):
        return True
...
def assert_has_shape(t: JTensor, shape: Sequence[int]) -> None:
    ...
    for i in range(t.ndim):
        if shape[i] != -1:
            asserts.eq(t.shape[i], shape[i], value_str1=value_str1, value_str2=value_str2)


def compatible_hparams(
    hparams1: Union[base_hyperparams.HParams, pax_fiddle.Config],
    hparams2: Union[base_hyperparams.HParams, pax_fiddle.Config],
) -> bool:
    ...
    if isinstance(p1, pax_fiddle.Config) or isinstance(p2, pax_fiddle.Config):
        if not (isinstance(p1, pax_fiddle.Config) and isinstance(p2, pax_fiddle.Config)):
            raise ValueError(
                "Expected hparams1 and hparams2 to have the same type; "
...
def get_template_fields(template: pax_fiddle.Config) -> List[str]:
    ...
    if isinstance(template, pax_fiddle.Config):
        return list(fdl.ordered_arguments(template, include_defaults=True, include_unset=True))
    else:
