...
def extract_block_context(
    ...
    if left_context < 1 or left_context > block_size + 1:
        raise ValueError(
            "left_context must be at least 1 and at most block_size + 1 = {}, got {}".format(
                block_size + 1, left_context
            )
        )
    if right_context < 0 or right_context > block_size:
        raise ValueError(
            "right_context must be at least 0 and at most block_size = {}, got {}".format(block_size, right_context)
        )
...
def _make_local_mask(seq_len: int, block_size: int, left_context: int, right_context: int) -> JTensor:
    ...
    src_positions = jnp.reshape(jnp.arange(num_blocks * block_size), [num_blocks, block_size])
    ...
    tgt_positions = block_start_positions[:, jnp.newaxis] + relative_context_positions[jnp.newaxis, :]
    # [num_blocks, block_size, context_size]: position differences between source-
    # target pairs.
    position_diff = src_positions[:, :, jnp.newaxis] - tgt_positions[:, jnp.newaxis, :]
    # [num_blocks, block_size, context_size]: if attention is allowed between
    # source-target pairs.
    valid_atten = jnp.logical_and(-right_context <= position_diff, position_diff < left_context)
    ...
    valid_atten &= jnp.logical_and(valid_src[:, :, jnp.newaxis], valid_tgt[:, jnp.newaxis, :])
...
class RelativeBias(base_layer.BaseLayer):
    ...
    def setup(self) -> None:
        ...
        pc = WeightHParams(shape=[self.num_heads, self.relative_attention_num_buckets], init=init)
    ...
    def _relative_position_bucket(self, relative_position: JTensor) -> JTensor:
        ...
        max_distance = jnp.array(self.relative_attention_max_distance).astype(self.dtype)
        ret = 0
        n = -relative_position
        ...
        val_if_large = max_exact + (
            jnp.log(n.astype(self.dtype) / max_exact) / jnp.log(max_distance / max_exact) * (num_buckets - max_exact)
        ).astype(jnp.int32)
    ...
    def __call__(
        ...
        if self.use_length_as_position and not self.do_eval:
            ...
            relative_position = jnp.expand_dims(key_segment_pos, -2) - jnp.expand_dims(query_segment_pos, -1)
        ...
        relative_bias = jnp.einsum("NX,BTSX->BNTS", self.theta.wrb, relative_bucket_one_hot)
    ...
    def extend_step(self, seq_length: int, time_step: Optional[Union[int, JTensor]] = None) -> JTensor:
...
class AttentionProjection(base_layer.BaseLayer):
    ...
    def setup(self) -> None:
        ...
        pc = WeightHParams(
            ...
            fan_in_axes=(fan_in_axes if self.explicit_fan_in_fan_out_axes else None),
            fan_out_axes=(fan_out_axes if self.explicit_fan_in_fan_out_axes else None),
        )
        # lsp： qkv调用这创建w参数
    ...
    def setup(self) -> None:
        ...
        if wt is not None:
            ...
            if self.attention_combine_dims:
                ...
                assert len(wt) == 3, "wp.wt only specifies the sharding for the last three dims of the weight tensor."
...
class CausalDepthwiseConv1D(base_layer.BaseLayer):
    ...
    def setup(self) -> None:
        assert self.name
        assert isinstance(self.hidden_dims, (list, tuple)) or isinstance(self.hidden_dims, int)
    ...
    def __call__(self, inputs: JTensor, axis: int, segment_pos: Optional[JTensor] = None) -> JTensor:
    ...
    def extend_step(
        ...
        Returns:
          ...
          on the sequence.
        """
        get_single_slice_at_index = functools.partial(jax.lax.dynamic_slice_in_dim, inputs, slice_size=1, axis=axis)
...
class DotProductAttention(base_layer.BaseLayer):
    ...
    rotary_position_emb_tpl: Optional[LayerTpl] = template_field(embedding_softmax.RotaryPositionalEmbedding)
    ...
    # subclass.
    def _create_rotary_position_emb(self, layer_tpl: LayerTpl, dim_per_head: int) -> None:
    ...
    def setup(self) -> None:
        ...
        if self.combine_qkv:
            ...
            self.create_child("combined_qkv", combined_qkv_project_input(query_input_dim))
        else:
            # lsp：qkv的线性层是AttentionProjection类，不是Linear类
        ...
        if self.use_rotary_position_emb:
            self._create_rotary_position_emb(self.rotary_position_emb_tpl, dim_per_head)
        ...
        if self.output_proj_use_nhd_shape and isinstance(wp.proj, (list, tuple)) and len(wp.proj) == 3:
            permutation = [1, 2, 0]
            post_proj_p.weight_split_dims_mapping.wt = [wp.proj[i] for i in permutation]
        else:
    ...
    def wrap_flash_attention(self, query, key, value, decoder_segment_ids):
    ...
    def _dot_atten(
        ...
        """
        query = self._shard_blnh(query)  # ((replica, data), None, mdl, None)
        key = self._shard_blnh(key)  # ((replica, data), None, mdl, None)
        value = self._shard_blnh(value)  # ((replica, data), None, mdl, None)
        ...
        # alibi mask: n * qlen * klen
        if alibi_mask is not None:
            logging.info(f"alibi_mask: {alibi_mask.shape}")
        # attn mask: bsz * 1 * qlen * klen
    ...
    def _dot_atten_one_step(
        ...
        if q_b != k_b:
            if q_b % k_b != 0:
                raise ValueError(f"q batch size {q_b} is not divisible by state batch size {k_b}")
    ...
    def __call__(
        ...
        if self.combine_qkv:
            ...
            # lsp: bsz, len, model_dim, , params: model_dim, n_head, head_dim -> bsz, len, n_head, head_dim
    ...
    def _fprop_update_decode_state(self, name: str, value: JTensor) -> None:
        ...
        if not self.is_mutable_collection(base_layer.DECODE_CACHE) or not self.decode_cache:
            return
        self.update_decode_state(name, value)

    @nn.nowrap
    def extend_decode_state(self, name: str, value: JTensor, time_step: JTensor, time_dim: int) -> JTensor:
        ...
        new_state = jax.lax.dynamic_update_slice(state, extend_value.astype(state.dtype), indices)
    ...
    def extend_step(
        ...
        *,
        ...
        if self.combine_qkv:
            # Project inputs to key, value and query using a combined weight for
            # faster performance on TPU.
            query_proj, key_proj, value_proj = self.combined_qkv.extend_step(query_vec, time_step=time_step)
        else:
        ...
        def _extend_decode_state_and_shard(name: str, extend_value: JTensor) -> JTensor:
            extended_state = self.extend_decode_state(name, extend_value, time_step, time_dim=time_dim)
        ...
        if self.dconv_qkv:
            ...
            query_proj = self.dconv_q.extend_step(query_state, axis=time_dim, step=time_step, segment_pos=segment_pos)
            if not is_cross_attention:
                key_proj = self.dconv_k.extend_step(key_state, axis=time_dim, step=time_step, segment_pos=segment_pos)
        ...
        # Paper: https://openreview.net/forum?id=GxjCYmQAody.
        if self.ngrammer_tpl is not None:
            encoded_state = _extend_decode_state_and_shard("encoded_pre_ngrammer", encoded)
    ...
    def transform_decode_state(self, transform_fn: base_layer.DecodeStateTransformFn):
    ...
    def lazy_broadcast_prefix(self, num_suffix_samples: int, suffix_length: int) -> None:
        """Performs lazy prefix broadcast on the decoding states."""
        raise NotImplementedError("lazy_broadcast_prefix not implemented, use DotProductAttentionWithLPB instead.")
    ...
    def right_align_decode_state_with_prefix(
        ...
        raise NotImplementedError(
            "right_align_decode_state_with_prefix not implemented, use DotProductAttentionWithLPB instead."
        )
...
class DotProductAttentionWithLPB(DotProductAttention):
    ...
    def lazy_broadcast_prefix(self, num_suffix_samples: int, suffix_length: int) -> None:
        ...
        for name, state in self.variables[base_layer.DECODE_CACHE].items():
            ...
            self.put_variable(PREFIX_DECODE_CACHE, f"{name}_{prev_pfx_count}_pfx", state)
            suffix_shape = (
    ...
    def _broadcast_prefix_length(self):
        ...
        for i in range(self._broadcast_prefixes_count):
            prefix_length += self.get_variable(PREFIX_DECODE_CACHE, f"key_state_{i}_pfx").shape[i + 1]
        return prefix_length

    def decoding_state_sequence_length(self):
        """Returns the length of full decoding sequences including prefixes."""
        key_state_length = self.get_decode_state("key_state").shape[1 + self._broadcast_prefixes_count]
    ...
    def _vmap_on_broadcast_prefixes(
        ...
        broadcast_dim_sizes = self.get_decode_state("key_state").shape[1 : 1 + self._broadcast_prefixes_count]
    ...
    def _run_with_all_decode_state_chunks(
        ...
        for i in range(pfx_count + 1):
            ...
            if i == pfx_count:
                ...
                states = [self.get_variable(PREFIX_DECODE_CACHE, f"{s}_{i}_pfx") for s in state_names]
            # Run one chunk with vmaps.
            results.append(
                self._vmap_on_broadcast_prefixes(fn, i, args_time_dims, broadcast_args_time_dims)(
    ...
    def _decode_state_chunk_length(self, chunk_id: int) -> int:
        ...
        return self.get_variable(PREFIX_DECODE_CACHE, f"key_state_{chunk_id}_pfx").shape[t_dim]
    ...
    def _slice_decode_chunk(self, x: JTensor, chunk_id: int, dim: int) -> JTensor:
        ...
        for i in range(min(pfx_count, chunk_id)):
            t_dim = i + 1
            start += self.get_variable(PREFIX_DECODE_CACHE, f"key_state_{i}_pfx").shape[t_dim]
        limit = start + self._decode_state_chunk_length(chunk_id)
        return jax.lax.slice_in_dim(x, start, limit, axis=dim)

    def _left_concat_decode_state(self, state_name: str, max_prefix_size: int) -> JTensor:
        ...
        for i in range(pfx_count):
            ...
            pfx = self.get_variable(PREFIX_DECODE_CACHE, f"{state_name}_{chunk_id}_pfx")
            pfx_len = pfx.shape[chunk_id + 1]
            subwindow_len = min(pfx_len, prefix_window_size)
            prefix_window_size -= subwindow_len
            pfx = jax.lax.slice_in_dim(pfx, pfx_len - subwindow_len, pfx_len, axis=chunk_id + 1)
            pfx = jnp.reshape(
                pfx,
    ...
    def _dot_atten_one_step(
        ...
        if rb_batched:
            relative_bias = jnp.reshape(relative_bias, batch_dims + relative_bias.shape[1:])
        ...
        if self.attention_extra_logit is None:
            ...
            probs = jnp.exp(self._log_softmax_with_extra_logit(padded_logits)).astype(key_dtype)
        ...
        if self.zero_fully_masked:
            ...
            if not am_batched:
                fully_masked = jnp.reshape(fully_masked, (1,) * len(batch_dims) + fully_masked.shape[1:])
        ...
        *,
        ...
        if pfx_count > 0:
            ...
            if segment_pos is not None:
                segment_pos = jnp.reshape(segment_pos, batch_dims + segment_pos.shape[1:])
        ...
        query_proj, key_proj, value_proj = _vmap_no_state(_proj_qkv)(self, query_vec)
        ...
        if self.dconv_qkv:
            if not extend_one_step:
                raise NotImplementedError("DotProductAttentionWithLPB does not support extend n steps with dconv.")
            # Update query in cache.
            ...
            def _dconv(layer, q, k, v, pos):
                ...
                query_proj = layer.dconv_q.extend_step(q, axis=t_dim, step=ts, segment_pos=pos)
                key_proj = layer.dconv_k.extend_step(k, axis=t_dim, step=ts, segment_pos=pos)
                value_proj = layer.dconv_v.extend_step(v, axis=t_dim, step=ts, segment_pos=pos)
                return query_proj, key_proj, value_proj
        ...
        if self.use_rotary_position_emb:
            ...
            query_proj, key_proj = _vmap_no_state(_rotary)(self, query_proj, key_proj, position)
        ...
        if self.relative_bias_tpl:
            ...
            if not extend_one_step:
                raise NotImplementedError(
                    "DotProductAttentionWithLPB does not support extend n steps with relative bias."
                )
            relative_bias = self.relative_bias.extend_step(
                seq_length=self.decoding_state_sequence_length(), time_step=time_step
            )
        ...
        if self.ngrammer_tpl is not None:
            if pfx_count > 0:
                raise NotImplementedError("ngrammer does not yet support lazy prefix broadcast")
            encoded_state = _extend_decode_state_and_shard("encoded_pre_ngrammer", encoded)
            # TODO(pax-dev): May need to fix segment_pos.
            encoded = self.ngrammer.extend_step(encoded_state, step=time_step, merge_heads=False)
...
def create_relative_positional_embedding(
    ...
    u_pc = WeightHParams(shape=[layer.num_heads, dim_per_head], init=WeightInit.Constant(0.0))
    v_pc = WeightHParams(shape=[layer.num_heads, dim_per_head], init=WeightInit.Constant(0.0))
...
class DotProductAttentionXL(DotProductAttention):
    ...
    def _rel_position_bias(self, content: JTensor, abs_pos_emb: JTensor) -> JTensor:
    ...
    def init_states(
        ...
        raise NotImplementedError("init_states is not implemented for %s" % self.__name__)
...
class LocalSelfAttention(DotProductAttention):
    ...
    def _dot_atten(
        ...
        if relative_bias is not None:
            raise NotImplementedError("relative bias for localattention is not supported yet")

        block_size = self.block_size
        if block_size is None and self.left_context is not None and self.right_context is not None:
        ...
        if atten_mask.shape[2] == 1:
            ...
            # -> [B, N, U, W, C]
            mask = jnp.tile(jnp.reshape(mask_block_context, [b, 1, u, 1, c]), [1, n, 1, w, 1])
        else:
            # Full attention mask
        ...
        if self.zero_fully_masked:
            # Return zeros for tokens which don't attend anything.
            fully_masked = jnp.all(mask < minus_inf / 2, axis=-1)[:, 0, :, :, jnp.newaxis, jnp.newaxis]
            encoded *= 1 - fully_masked
...
class LocalSelfAttentionXL(LocalSelfAttention):
    ...
    def _atten_logits(self, query, key):
        ...
        term_bd = jnp.pad(term_bd, ((0, 0), (0, 0), (0, 0), (0, c - w), (0, c + 1 - f)))
    ...
    def extend_step(
        ...
        *,
        ...
        raise NotImplementedError("extend_step is not implemented for %s" % self.__name__)
