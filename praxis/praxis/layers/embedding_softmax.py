...
class TokenCounter(base_layer.BaseLayer):
    ...
    def setup(self) -> None:
        ...
        self.create_variable("approx_total_tokens_mm", approx_total_tokens_mm, trainable=False)
    ...
    def __call__(self, inputs: JTensor, paddings: JTensor) -> JTensor:
        ...
        if not self.do_eval:
            ...
            self.update_var(
                "approx_total_tokens_mm", new_approx_total_tokens_mm
...
class Embedding(base_layer.BaseLayer):
    ...
    def emb_lookup(self, ids: JTensor) -> JTensor:
        ...
        embs = base_layer.maybe_shard(embs, ap.emb_out_split_dims_mapping, self.mesh_axis_names)
        return embs
...
class FullSoftmax(base_layer.BaseLayer):
    ...
    def _compute_logits_and_loss(self, inputs, class_weights,
        ...
        if self.loss_batch_mean:
            # lsp
            logging.info(f'============loss_batch_mean: {loss_batch_mean}==========')
...
class SigmoidCrossEntropy(base_layer.BaseLayer):
    ...
    def __call__(
        ...
        if class_weights.shape[-1] == self.num_classes:
            ...
            per_class_weight = jnp.ones(class_weights.shape[:-1] + (self.num_classes,))
        ...
        per_class_xent = (
            relu_logits - logits * class_probabilities + jnp.log1p(jnp.exp(neg_abs_logits))
        )
        per_example_xent = jnp.sum(per_class_xent * per_class_weight, axis=-1, dtype=jnp.float32)
...
class GShardSharedEmbeddingSoftmax(base_layer.BaseLayer):
    ...
    def __call__(
        ...
        if self.label_smoothing_prob > 0.0 and not self.do_eval:
            ...
            class_probabilities = (
                on_value * class_probabilities + off_value * (1.0 - class_probabilities)
            ).astype(jnp.float32)
        class_probabilities = jax.lax.stop_gradient(class_probabilities)
        ...
        if self.use_tgt_labels_size_as_loss_denominator:
            loss_denominator = jnp.sum(jnp.ones_like(class_weights), dtype=jnp.float32)
        else:
        ...
        if class_probabilities_prior_to_label_smoothing is not None:
            ...
            total_xent_prior_to_label_smoothing = jnp.sum(
                jnp.expand_dims(per_example_xent_prior_to_label_smoothing, axis=-1) * class_weights,
                dtype=jnp.float32,
            )
            avg_xent_prior_to_label_smoothing = (
                total_xent_prior_to_label_smoothing / loss_denominator
            ).astype(inputs_dtype)
            self.add_summary("avg_xent_prior_to_label_smoothing", avg_xent_prior_to_label_smoothing)
...
class PositionalEmbedding(base_layer.BaseLayer):
    ...
    def __call__(
        ...
        scaled_time = position[:, :, jnp.newaxis] * inv_timescales[jnp.newaxis, jnp.newaxis, :]
        signal = jnp.concatenate([jnp.sin(scaled_time), jnp.cos(scaled_time)], axis=2).astype(
            self.fprop_dtype
        )
        # Force usage of `np` rather than `jnp` to compute static values at trace
        # time.
        signal = jnp.pad(signal, [[0, 0], [0, 0], [0, np.mod(self.embedding_dims, 2)]])
        return signal
