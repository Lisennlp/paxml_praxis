...
def compute_xent_loss_helper(
    ...
    per_example_output = NestedMap(labels=labels, batch_weights=batch_weights, scores=predictions.per_sequence_xent)
...
class LanguageModel(base_model.BaseModel):
    ...
    def _prepare_decode_data(
        ...
        if "prefix_lengths" in input_batch:
            ...
            prefix_lengths = jnp.sum(input_batch.inputs_indicator.astype(jnp.int32), axis=1)
        else:
        ...
        if decoder_params.fprop_for_prefix:
            ...
            fprop_input_ids, fprop_input_paddings = sample_decode.right_align_prefix_ids(
            ...
            fprop_segment_ids = jnp.where(
                jnp.arange(max_prefix_len) < (max_prefix_len - prefix_lengths)[:, jnp.newaxis],
    ...
    def decode_with_params(
        ...
        if decoder_params.seqlen <= 0:
            raise ValueError(
                f"Must set p.decoder_tpl.seqlen > 0, current value = {decoder_params.seqlen}"
            )
        max_prefix_len = input_batch.ids.shape[1]
        ...
        if template_has_type(decoder_params, FlatBeamSearchHParams):
            ...
            with decoder_utils.maybe_decode_mesh_transpose(mdl_for_decode, decode_mesh_transpose):
            ...
            if decoder_params.override_next_token_sampler_params:
                ...
                next_token_sampler_p.top_k_recall_target = decoder_params.top_k_recall_target
                next_token_sampler_p.use_top_k_for_logprobs = decoder_params.use_top_k_for_logprobs
            ...
            result = sample_decode.sample_decode(
                ...
                extend_step_fn,
                transform_decode_state_fn,
                lazy_broadcast_prefix_fn if decoder_params.lazy_prefix_broadcast else None,
                next_token_sampler,
                decode_data.input_ids,
                decode_data.input_paddings,
                decode_data.seqlen,
    ...
    def process_decode_out(
        ...
        decoded_strs = input_obj.ids_to_strings(decode_out.output_ids, decode_out.decode_lengths)
        original_strs = input_obj.ids_to_strings(decode_out.ids, decode_out.original_lengths)
        prefix_strs = input_obj.ids_to_strings(decode_out.prefix_ids, decode_out.prefix_lengths)

        ret = []
        for idx, decoded_str in enumerate(decoded_strs):
            if "eval_sample_weights" in decode_out and not decode_out.eval_sample_weights[idx]:
        ...
        metrics = NestedMap(decoded_length=(decoded_lengths, np.array(1.0, np.float32)))
        out_clu_metrics = NestedMap()
...
class LanguageModelDPO(base_model.BaseModel):
    ...
    def _prepare_predict_data(self, input_batch: NestedMap) -> _TransformerLmInput:
        paddings = input_batch.paddings
        weights = input_batch.weights
        if self.apply_eval_sample_weights:
    ...
    def __call__(self, input_batch: NestedMap):
        ...
        r_hat_y_l = jax.lax.stop_gradient(self.beta * (y_l_pi_log_p - y_l_ref_log_p))
        r_hat_y_w = jax.lax.stop_gradient(self.beta * (y_w_pi_log_p - y_w_ref_log_p))

        # This is first equation on page #5 in the DPO paper.
        per_example_loss = (
            self.beta * jax.nn.sigmoid(r_hat_y_l - r_hat_y_w) * (y_l_pi_log_p - y_w_pi_log_p)
        )
        ...
        self.add_summary("p_correct_ranking", jnp.mean(jax.nn.sigmoid(r_hat_y_w - r_hat_y_l)))
...
class SequenceModel(base_model.BaseModel):
    ...
    def compute_predictions(self, input_batch):
        ...
        labels = NestedMap(class_ids=input_batch.tgt.labels, class_weights=input_batch.tgt.weights)
    ...
    def compute_loss(self, predictions, input_batch):
        ...
        return compute_xent_loss_helper(predictions, input_batch.tgt, self.return_predictions)
    ...
    def decode_with_params(
        ...
        if template_has_type(decoder_params, FlatBeamSearchHParams):
            ...
            fprop_input_paddings = jnp.ones((batch_size, 1), input_batch.tgt.paddings.dtype)
            result = beam_search.beam_search(
                self,
                extend_step_fn,
                fprop_fn,
    ...
    def process_decode_out(
        ...
        if num_samples > 1:
            sampled_ids = np.reshape(decode_out.output_ids, [-1, max_len])
            sampled_lengths = np.reshape(decode_out.decode_lengths, [-1])
            sampled_strs = input_obj.ids_to_strings(sampled_ids, sampled_lengths, key="tgt")
            sampled_strs = np.reshape(sampled_strs, [batch_size, num_samples])
        else:
            sampled_strs = None
        ...
        source_lengths = np.sum(1.0 - decode_out.src.paddings, axis=1).astype(np.int32)
        source_strs = input_obj.ids_to_strings(decode_out.src.ids, source_lengths, key="src")
        target_lengths = np.sum(1.0 - decode_out.tgt.paddings, axis=1).astype(np.int32)
        target_strs = input_obj.ids_to_strings(decode_out.tgt.ids, target_lengths, key="tgt")
        ret = list()
        for idx, decoded_str in enumerate(decoded_strs):
            if (
        ...
        metrics = NestedMap(decode_length=(decode_lengths, np.array(1.0, np.float32)))
        out_clu_metrics = NestedMap()
        return metrics, ret, out_clu_metrics
...
class ClassificationModel(base_model.BaseModel):
    ...
    def compute_loss(  # pytype: disable=signature-mismatch  # jax-ndarray
        ...
        per_example_out = NestedMap(labels=label_probs, scores=predictions.softmax_output.logits)
...
class BertModel(base_model.BaseModel):
    ...
    def compute_predictions(self, input_batch: NestedMap) -> Predictions:
        ...
        if self.label_smoothing_prob > 0.0:
            ...
            labels = NestedMap(class_probabilities=class_probabilities, class_weights=augmented_pos)
        else:
            # Only compute loss on masked pos.
    ...
    def compute_loss(  # pytype: disable=signature-mismatch  # jax-ndarray
        ...
        num_seqs = jnp.sum(jnp.amax(input_batch.segment_ids.astype(jnp.float32), axis=1))
