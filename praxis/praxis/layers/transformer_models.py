...
def _set_embedding_softmax_sharding_params_for_transformers(
    ...
    *,
    ...
    if fdl.get_callable(embedding_softmax_p) == embedding_softmax.GShardSharedEmbeddingSoftmax:
        ...
        or "RecordingSharedEmbeddingSoftmax" in str(fdl.get_callable(embedding_softmax_p))
        ...
        raise NotImplementedError(
            f"embedding_softmax class {fdl.get_callable(embedding_softmax_p)} not supported"
        )

    if (
        fdl.get_callable(embedding_softmax_p) == embedding_softmax.GShardSharedEmbeddingSoftmax
        or issubclass(
            fdl.get_callable(embedding_softmax_p),
            embedding_softmax.SharedEmbeddingSoftmax,
        )
        ...
        (embedding_softmax_p.activation_split_dims_mapping.emb_out_split_dims_mapping) = a_bld
...
def _set_stacked_transformer_sharding(
    ...
    *,
    ...
    if stacked_p.moe_layer_tpl is not None:
        ...
        moe_ap.gs = [data, None]
        # dispatch and combine tensors
        moe_ap.gsec = [data, None, None, None]
        moe_ap.gecs = [data, None, None, None]
        moe_ap.gec = [data, None, None] # top2没用到
        moe_ap.gsm = [data, None, 'mdl']

        moe_ap.egch = [None, data, None, 'mdl']
        moe_ap.egcm = [None, data, None, 'mdl']
        moe_ap.gecm = [data, None, None, 'mdl']
...
# lsp: model_p.lm_tpl: TransformerLm, 然后该类分配subclass： stacked_transformer_tpl
# lsp forward model class -> setup
class TransformerLm(base_layer.BaseLayer):
    ...
    post_attention_ngrammer_tpls: Optional[Sequence[LayerTpl]] = template_field(None)
    ...
    def set_sharding_params_v1(
        ...
        *,
        ...
        if batch_axes is None:
            batch_axes = (replica_axis, data_axis)  # lsp：等价于dp, fsdp
        # lsp: training_optimized: True
        bld = [batch_axes, None, mdl_axis] if training_optimized else [batch_axes, None, None]
        egcm = (
        ...
        # a_bld: sharding of output of ffn/attention, shape (b, l, d). -> ffn/aten output, merge head之后 (dp, fsdp), None, mp
        a_bld = bld
        # a_blf: sharding of output of ffn0, shape (b, l, f). -> ffn0的输出
        a_blf = [batch_axes, None, mdl_axis]
        # a_blnh: sharding of the attention activation of shape (b, l, num_heads,
        # per_head_size). -> lsp: atten score shard ((dp, fsdp), None, mp, None)
        a_blnh = [batch_axes, None, mdl_axis, None]
        # a_blv: sharding of the logits activation of shape (b, l, vocab_size). lsp: logits输出，((dp, fsdp), None, mp)
        ...
        a_egcm = egcm
        # bld: (replica, data), None, mdl ||  blf: (replica, data), None, mdl || blnh: (replica, data), None, mdl, None
        # blv:  (replica, data), None, None, mdl  , w_vd: mdl, data |
    ...
    def set_custom_sharding_params(
        ...
        *,
        ...
        if (
            ...
            and lm_p.position_emb_tpl.cls == embedding_softmax.TrainablePositionalEmbedding
        ):
            lm_p.position_emb_tpl.weight_split_dims_mapping.wt = pos_emb_w_ld
            (lm_p.position_emb_tpl.activation_split_dims_mapping.emb_out_split_dims_mapping) = a_bld
        ...
        if lm_p.stacked_transformer_tpl.cls == transformers.PipelinedTransformer:
            ...
            lm_p.stacked_transformer_tpl = _set_transformer_sharding(lm_p.stacked_transformer_tpl)

        if lm_p.separate_embedding_tpl is not None:
            lm_p.separate_embedding_tpl = _set_embedding_softmax_sharding_params_for_transformers(
    ...
    def _create_alibi_position_emb(self, layer_tpl: LayerTpl, num_heads: int) -> None:
    ...
    def setup(self) -> None:
        ...
        # Transformer layers. lsp：前传call函数self.stacked_transformer_tpl
        ...
        # lsp: StackedTransformerRepeated, block，一层
        ...
        assert xformer_params.model_dims == 0 or xformer_params.model_dims == self.model_dims
        ...
        # lsp forward-0  -> self.stacked_transformer_tpl -> model_params.py-657
        # model_p.lm_tpl.stacked_transformer_tpl = pax_fiddle.Config(layers.StackedTransformerRepeated)
    ...
    # lsp
    def compute_loss(self, activations: JTensor, labels: Optional[NestedMap] = None) -> NestedMap:
        ...
        if labels is None:
            ...
            xent_output.log_probs = jax.nn.log_softmax(casted_logits).astype(logits_dtype)
        ...
        **input_kwargs,
        ...
        if self.ngrammer_tpl is not None:
            ...
            if segment_pos is not None:
                ...
                self.update_decode_state("ngrammer_prefix_segment_pos", ngrammer_prefix_segment_pos)

        if segment_mask is None:
            if self.model_type == LanguageModelType.BIDIRECTIONAL:
                segment_mask = attentions.segment_mask(segment_ids, segment_ids, inputs.dtype)
            else:
    ...
    def _emb_lookup(self, input_ids):
        ...
        if self.separate_embedding_tpl is not None:
            # [B, ?, D]
            input_emb = self.embedding_lookup.extend_step(input_ids, time_step=time_step)
        else:
            # [B, ?, D]
    ...
    def _emb_ngrammer(self, input_ids, input_emb, segment_pos):
        ...
        ngrammer_segment_pos_prefix = self.get_decode_state("ngrammer_prefix_segment_pos")
        input_ids = jnp.concatenate([ngrammer_prefix, input_ids], axis=1)
        input_emb = jnp.concatenate([ngrammer_prefix_emb, input_emb], axis=1)
        if segment_pos is not None:
            segment_pos = jnp.concatenate([ngrammer_segment_pos_prefix, segment_pos], axis=1)
    ...
    def extend_step(
        ...
        if segment_pos is not None:
            # [B, T]
            segment_pos = segment_pos[:, jnp.newaxis] if is_single_token else segment_pos
        ...
        if self.ngrammer_tpl is not None:
            if is_single_token:
                ...
                self.update_decode_state("ngrammer_prefix", input_ids[:, -1][:, jnp.newaxis])
                self.update_decode_state("ngrammer_prefix_emb", input_emb[:, -1][:, jnp.newaxis])
                if segment_pos is not None:
                    self.update_decode_state(
    ...
    def transform_decode_state(self, transform_fn: base_layer.DecodeStateTransformFn) -> None:
    ...
    def lazy_broadcast_prefix(self, num_suffix_samples: int, suffix_length: int) -> None:
        ...
        for name, state in self.variables[base_layer.DECODE_CACHE].items():
            if name.startswith("ngrammer"):
                ...
                new_state_shape = (num_suffix_samples * state.shape[0],) + state.shape[1:]
    ...
    def right_align_decode_state_with_prefix(
        ...
        self.transformer.right_align_decode_state_with_prefix(max_prefix_size, right_align_fn)
...
class TransformerEncoderDecoder(base_layer.BaseLayer):
    ...
    position_emb_tpl: Optional[LayerTpl] = template_field(embedding_softmax.PositionalEmbedding)
    ...
    encoder_post_attention_ngrammer_tpls: Optional[Sequence[LayerTpl]] = template_field(None)
    ...
    decoder_post_attention_ngrammer_tpls: Optional[Sequence[LayerTpl]] = template_field(None)
    ...
    def set_sharding_params_v1(
        ...
        *,
        ...
        if (
            ...
            and model_p.position_emb_tpl.cls == embedding_softmax.TrainablePositionalEmbedding
        ):
        ...
        if (
            ...
            model_p.encoder_position_emb_tpl.weight_split_dims_mapping.wt = pos_emb_w_ld

        model_p.softmax_tpl = _set_embedding_softmax_sharding_params_for_transformers(
            model_p.softmax_tpl,
            w_vd=w_vd,
            a_blv=a_blv,
            a_bld=a_bld,
        ...
        if model_p.encoder_embedding_tpl is not None:
            model_p.encoder_embedding_tpl = _set_embedding_softmax_sharding_params_for_transformers(
        ...
        if (
            ...
            model_p.decoder_position_emb_tpl.weight_split_dims_mapping.wt = pos_emb_w_ld
        ...
        if model_p.decoder_embedding_tpl is not None:
            model_p.decoder_embedding_tpl = _set_embedding_softmax_sharding_params_for_transformers(
    ...
    def _validate_encoder_mask_self_attention(self, mask_self_attention: bool) -> None:
        """This method can be overridden to remove the check."""
        if mask_self_attention:
            raise ValueError("Encoder attention should be un-masked in TransformerEncoderDecoder.")

    def _validate_decoder_mask_self_attention(self, mask_self_attention: bool) -> None:
        """This method can be overridden to remove the check."""
        if not mask_self_attention:
            raise ValueError("Decoder attention should be masked in TransformerEncoderDecoder.")
    ...
    def setup(self) -> None:
        ...
        def set_model_dims_and_packing(stacked_transformer_tpl, model_dims, packed_input):
            if issubclass(
            ...
            elif fdl.get_callable(stacked_transformer_tpl) == transformers.PipelinedTransformer:
                assert (
                ...
                raise ValueError(f"{fdl.get_callable(stacked_transformer_tpl)} not supported.")
        ...
        if self.encoder_position_emb_tpl is not None:
            ...
            encoder_position_emb_tpl = self._clone_layer_params(self.encoder_position_emb_tpl)
        ...
        if self.encoder_stacked_transformer_tpl is None:
            raise ValueError("Encoder stack must be specified for TransformerEncoderDecoder.")

        # Use the user specified StackedTransformer for the encoder, assuming
        # everything is set up appropriately.
        encoder_params = self._clone_layer_params(self.encoder_stacked_transformer_tpl)
        set_model_dims_and_packing(encoder_params, self.model_dims, self.packed_input)
        ...
        elif issubclass(encoder_params.cls, transformers.StackedTransformerRepeated):
        ...
        if self.decoder_position_emb_tpl is not None:
            ...
            decoder_position_emb_tpl = self._clone_layer_params(self.decoder_position_emb_tpl)
        ...
        if self.decoder_stacked_transformer_tpl is None:
            raise ValueError("Decoder stack must be specified for TransformerEncoderDecoder.")

        # Use the user specified StackedTransformer for the decoder, assuming
        # everything is set up appropriately.
        decoder_hparams = self._clone_layer_params(self.decoder_stacked_transformer_tpl)
        set_model_dims_and_packing(decoder_hparams, self.model_dims, self.packed_input)
        ...
        elif issubclass(decoder_hparams.cls, transformers.StackedTransformerRepeated):
        ...
        # Final softmax.
        softmax_params = self.softmax_tpl.clone()
        assert softmax_params.input_dims == 0 or softmax_params.input_dims == self.model_dims
        softmax_params.input_dims = self.model_dims
        self.create_child("softmax", softmax_params)
    ...
    def encode(
        ...
        if self.position_emb_tpl is not None:
            position_emb = self.position_emb(seq_length=seq_length, position=input_segment_pos)
        ...
        if input_segment_mask is None:
            input_segment_mask = attentions.segment_mask(input_segment_ids, dtype=input_emb.dtype)
    ...
    def compute_loss(self, activations: JTensor, labels: Optional[NestedMap] = None) -> NestedMap:
        ...
        per_token_xent = xent_output.per_example_xent * labels.class_weights.astype(jnp.float32)
        xent_output.per_token_xent = per_token_xent
        xent_output.per_sequence_xent = jnp.sum(per_token_xent, -1, dtype=jnp.float32)
    ...
    def extend_step(self, targets: JTensor) -> NestedMap:
        ...
        time_step = self.get_decode_state("time_step")
        if self.decoder_ngrammer_tpl is not None:
            target_emb = self.decoder_ngrammer(targets, target_emb, paddings=None, segment_pos=None)
        ...
        if self.position_emb_tpl is not None:
            target_position_emb = self.position_emb(seq_length=1, position=segment_pos)
            target_emb += target_position_emb
        elif self.decoder_position_emb_tpl is not None:
            target_position_emb = self.decoder_position_emb(seq_length=1, position=segment_pos)
            target_emb += target_position_emb
