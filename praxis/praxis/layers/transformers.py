...
def compute_attention_masks_for_extend_step(
    ...
    attention_mask = jnp.squeeze(attentions.convert_paddings_to_mask(causal_padding), axis=1)
...
def _get_sentence_embeddings(inputs: JTensor, segment_ids: JTensor) -> JTensor:
    ...
    segment_sum = jax.ops.segment_sum(inputs, segment_ids, num_segments=max_segments)
    ...
    segment_mean = segment_sum / jnp.maximum(num_elements_per_segment[:, jnp.newaxis], 1)
...
class TransformerFeedForward(base_layer.BaseLayer):
    ...
    activation_tpl: pax_fiddle.Config[activations_lib.BaseActivation] = template_field(
        activations_lib.ReLU
    )
    ...
    def setup(self) -> None:
        ...
        if self._is_ffn1_gated:
            activation = pax_fiddle.Config(activations_lib.Identity)  # lsp: 直接返回结果，不做任何操作
        ...
        if self._is_ffn1_gated:
            ...
            gate_p.activation_split_dims_mapping.out = (
                ap.ffn0
            )  # shard: a_blf: ((replica, data), None, mdl)
            if self.internal_gshard_variance_scaling_fan_in_init:
                scale = (1.0 / self.input_dims) ** 0.5 * (3.0**0.5)
        ...
        ffn2_p.weight_split_dims_mapping.wt = wp.ffn1  # (mdl, data)
        ffn2_p.activation_split_dims_mapping.out = (
            ap.ffn1
        )  # # shard: a_bld: ((replica, data), None, mdl)
...
class TransformerFeedForwardMoe(base_layer.BaseLayer):
    ...
    def _count_dead_neurons(self, hidden, dispatch_tensor):
        ...
        if isinstance(self.activation_tpl.cls, activations_lib.GELU):
            logging.info("Setting dead neuron count threshold=-3.0 for approximate GeLU activation")
            threshold = -3.0
        ...
        max_hidden = jnp.max(jnp.max(hidden_minus_ten_padding_indicator, axis=1), axis=1)
    ...
    def _combine_top2_expert_outputs(self, inputs, paddings, segment_ids):
        ...
        assert not self._is_ffn1_gated, "dense_top2 routing does not support gated MoE activations"
        ...
        gates = raw_gates * jnp.greater_equal(raw_gates, raw_gates_thr).astype(fprop_dtype)
    ...
    def _dispatch_and_combine_expert_outputs(self, inputs, paddings, segment_ids):
        ...
        num_groups = self.num_groups
        assert num_groups
        if self.min_group_size is not None and num_tokens / num_groups < self.min_group_size:
        ...
        if self.moe_gating_embedding_level == "sentence":
            ...
            # groups.
            reshaped_sentence_embeddings = sentence_embeddings.reshape([num_groups, g_len, m_dim])
            reshaped_sentence_embeddings = self._split(reshaped_sentence_embeddings, ap.gsm)
            logits = jnp.einsum("gsm,me->gse", reshaped_sentence_embeddings, self.theta.gate)
        else:
        ...
        if self.gating_func in ["top2", "expert_choice_v2"]:
            ...
            combined_output = jnp.einsum(
                "gecm,gecs,gec->gsm", transposed_expert_output, dispatch_tensor, combine_tensor
            )
        else:
            raise ValueError("Unsupported gating function: %s " % self.gating_func)
        combined_output = self._split(combined_output, ap.gsm)
...
class Transformer(base_layer.BaseLayer):
    ...
    def __call__(
        ...
        # norm_policy： pre || lsp： llama是在attention和ffn之前进行norm操作
        if self.norm_policy == "primer_hybrid":
    ...
    def right_align_decode_state_with_prefix(
        self, max_prefix_size: int, right_align_fn: base_layer.DecodeStateTransformFn
    ) -> None:
        """Right aligns decode state with prefix decode states."""
        self.self_attention.right_align_decode_state_with_prefix(max_prefix_size, right_align_fn)
...
class StackedTransformer(base_layer.BaseLayer):
    ...
    gating_func: str = "top2"
    unadjusted_expert_capacity_factor: float = 2.0
    transformer_layer_params_tpl: Union[LayerTpl, Sequence[LayerTpl]] = template_field(Transformer)
    packed_input: bool = False
    ...
    def setup(self) -> None:
        ...
        def _layer_params(i):
            ...
            p_i.residual_dropout_prob = self.residual_dropout_prob or self.dropout_prob
        ...
        if isinstance(self.transformer_layer_params_tpl, (list, tuple)):
            if self.num_layers % len(self.transformer_layer_params_tpl):
                raise ValueError("num_layers should be divisible by transformer_layer_params_tpl")
        ...
        if self.input_dropout_prob > 0.0:
            self.create_child(
                "input_dropout",
                pax_fiddle.Config(stochastics.Dropout, keep_prob=1.0 - self.input_dropout_prob),
            )
    ...
    # lsp forward-4: StackedTransformer
    def __call__(
        self,
        inputs: JTensor,
        paddings: JTensor,
        ...
        if atten_mask is None:
            if segment_pos is None:
                ...
                src_segment_ids = jnp.where(
                    jnp.arange(max_t)[jnp.newaxis, :] < time_step - segment_pos[:, jnp.newaxis],
                    0,
                    1,
                )
                # [B, 1, 1, T]
            ...
            _, cross_attention_mask = compute_attention_masks_for_extend_step(
                jnp.asarray(0, dtype=jnp.uint32), max_t, None, cross_paddings, cross_segment_mask
            )
    ...
    def right_align_decode_state_with_prefix(
        ...
        """Right aligns decode state with prefix decode states."""
        for layer in self.x_layers:
            layer.right_align_decode_state_with_prefix(max_prefix_size, right_align_fn)
...
class StackedTransformerRepeated(base_layer.BaseLayer):
    ...
    def setup(self) -> None:
        ...
        # pax_fiddle.Config是把第二及之后的参数传给第一个参数， repeats.Repeat的init函数，如果没有init，则直接初始化最外面的属性
        # lsp forward-2 -> repeats.Repeat -> repeats.py-119 -> setup -> self.sub = self.block == stacked_transformer_tpl
        # model_params.py-661 -> 613: stacked_transformer_tpl == pax_fiddle.Config(layers.StackedTransformer)
        ...
        # 创建repeat参数，在StackedTransformer的时候会调用
    ...
    # lsp: output = self.transformer(inputs, paddings, segment_mask=segment_mask, segment_pos=segment_pos)
    def __call__(
        self,
        inputs: JTensor,
        paddings: JTensor,
    ...
    def right_align_decode_state_with_prefix(
        ...
        """Right aligns decode state with prefix decode states."""
        self.repeat_layer.right_align_decode_state_with_prefix(max_prefix_size, right_align_fn)
...
class PipelinedTransformer(base_layer.BaseLayer):
    ...
    def setup(self) -> None:
        ...
        pipeline_params.weight_split_dims_mapping.stages = self.weight_split_dims_mapping.stages
    ...
    def __call__(
        ...
        if not self.stream_io:
            ...
            if bld_mapping is not None:
                ...
                paddings = base_layer.maybe_shard(paddings, bld_mapping[:-1], self.mesh_axis_names)
        ...
        if not self.stream_io:
            ...
            outputs = base_layer.maybe_shard(outputs, bld_mapping, self.mesh_axis_names)
