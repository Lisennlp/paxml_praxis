...
class Exponential(BaseSchedule):
    ...
    def __post_init__(self):
        ...
        self.linear = instantiate(
            pax_fiddle.Config(Linear, start=(x0, math.log(y0)), limit=(x1, math.log(y1)))
        )
...
class Cosine(BaseSchedule):
    ...
    def value_at(self, step: JTensor) -> JTensor:
        ...
        return self.final_value + 0.5 * decay_gap * (
            1
            + jnp.cos(
                math.pi * jnp.minimum(1.0, jnp.array(step, dtype=jnp.float32) / self.total_steps)
            )
        )
...
class DelayedCosine(BaseSchedule):
    ...
    def __post_init__(self):
        ...
        self.max = y0
        self.min = y1
        self.linear = instantiate(pax_fiddle.Config(Linear, start=(x0, 0), limit=(x1, math.pi)))
...
class LinearRampupPolynomialDecay(BaseSchedule):
    ...
    def __post_init__(self):
        ...
        assert self.decay_start >= self.warmup_steps, "decay_start must greater than warmup_steps."
        assert self.decay_end >= self.decay_start, "decay_end must be greater than decay_start"
        ...
        if self.warmup_steps > 0:
            self._schedules.append(
                instantiate(
                    pax_fiddle.Config(Linear, start=(0, 0.0), limit=(self.warmup_steps, self.max))
                )
            )
    ...
    def value_at(self, step: JTensor) -> JTensor:
        return jnp.array(
            optax.join_schedules([s.value_at for s in self._schedules], self._boundaries)(step),
            jnp.float32,
        )
...
class LinearRampupCosineDecay(BaseSchedule):
    ...
    def __post_init__(self):
        ...
        ), f"decay_start ({self.decay_start}) must greater than warmup_steps ({self.warmup_steps})."
        assert (
            self.decay_end >= self.decay_start
        ), f"decay_end ({self.decay_end}) must be greater than decay_start ({self.decay_start})"
        assert self.max > 0, "Must set max."
...
class PiecewiseConstant(BaseSchedule):
    ...
    def __post_init__(self):
        ...
        if self.boundaries is None or self.values is None:
            raise ValueError("The parameters `boundaries` and `values` must not be None.")
...
class Transformer(BaseSchedule):
    ...
    def value_at(self, step: JTensor) -> JTensor:
        ...
        warmup_steps = jnp.array(self.warmup_steps * self.worker_replicas, dtype=jnp.float32)
        if self.decay_end is not None:
            decay_end = jnp.array(self.decay_end, dtype=jnp.float32)
            current_step = jnp.where(current_step < decay_end, current_step, decay_end)
...
class SqrtDecay(BaseSchedule):
    ...
    def value_at(self, step: JTensor) -> JTensor:
        ...
        return jax.lax.rsqrt(jnp.maximum(current_step - offset, decay_start)) * multiplier
...
class LinearRampupPiecewiseConstant(BaseSchedule):
    ...
    def __post_init__(self):
        super().__post_init__()
        assert len(self.boundaries) >= 1 and len(self.boundaries) == len(self.values)
        self.p0 = instantiate(
            pax_fiddle.Config(Linear, start=(0, 0.0), limit=(self.boundaries[0], self.values[0]))
        )
        # Offset the boundaries, since each schedule passed to
        # optax.join_schedules() will receive a step count indicating the number
        # of steps since the previous boundary transition.
        ...
        self.p1 = instantiate(
            pax_fiddle.Config(PiecewiseConstant, boundaries=boundaries_pc, values=self.values)
        )

    def value_at(self, step: JTensor) -> JTensor:
        return jnp.array(
            optax.join_schedules([self.p0.value_at, self.p1.value_at], self.boundaries[:1])(step),
            jnp.float32,
        )
...
class PiecewiseSchedule(BaseSchedule):
    ...
    def value_at(self, step: JTensor) -> JTensor:
        return jnp.array(
            optax.join_schedules([s.value_at for s in self.schedules], self.boundaries)(step),
            jnp.float32,
        )
...
class CycleSchedule(BaseSchedule):
    ...
    def __post_init__(self):
        ...
        if len(self.schedules) != len(self.steps):
            raise ValueError(
                "len(schedules) != len(steps): %s vs %s" % (len(self.schedules), len(self.steps))
            )
        boundaries = [0]
    ...
    def value_at(self, step: JTensor) -> JTensor:
        ...
        for boundary, schedule in zip(self._boundaries, self.schedules[1:]):
            output = jnp.where(relative_step < boundary, output, schedule.value_at(step))
        return output
...
class ContinuousSchedule(BaseSchedule):
    ...
    def __post_init__(self):
        super().__post_init__()
        limit = self.start_step + self.half_life_steps * math.log(self.min) / math.log(0.5)
        self.exp: Exponential = instantiate(
            pax_fiddle.Config(Exponential, start=(self.start_step, 1.0), limit=(limit, self.min))
        )
